...
def getActfromState(state, h, n_point):

	a = []
	for s in range(1, n_point):
		if s not in h:
			a.append(s)
	return a
...
if __name__ == '__main__':
	...
	for point in range(1, 6):
		...
		for _ in range(n_point):
			...
			a = getActfromState(state, h, n_point)
			if e <= 0.95:
				a = np.argmax(a)
			else:
				a = np.random.choice(a)
			Q[state, a] = calc_distance(model.iloc[state,:], model.iloc[a,:]) + GAMMA * Q[state, a]
			print(state)
			print(a)
			h.append(a)
